...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    if stored_cwd and stored_cwd != str(resolve_agent_cwd()):
        return False
    # Multiplex gateway profile check (fork). Unlike Model/Provider above this is NOT symmetric: a
    # missing stored "Profile:" line is also a mismatch when the agent runs under a named profile,
    # because a prompt cached before the field existed (or while profile routing was broken) may
    # carry another profile's identity. Single-profile gateways never set agent.profile (no-op).
    # Only a real string counts: agent.profile is str-or-None by contract, and MagicMock-based

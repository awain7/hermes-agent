...
def _timestamp_line(agent: Any) -> str:
    ...
    # Multiplex gateway profile (fork): lets conversation_loop._stored_prompt_matches_runtime() reject a
    # stored prompt built under another profile instead of silently reusing its identity. Only a real
    # string counts: agent.profile is str-or-None by contract (agent_init), and MagicMock-based test
    # agents must not stamp a mock repr here.
    _profile = getattr(agent, "profile", None)
    _profile = _profile.strip() if isinstance(_profile, str) else ""
    trailer = (("Session ID", agent.session_id if agent.pass_session_id else None), ("Model", agent.model),
               ("Provider", agent.provider), ("Platform", agent.platform), ("Profile", _profile or None))

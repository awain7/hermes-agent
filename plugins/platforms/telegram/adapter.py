...
def check_telegram_requirements() -> bool:
    ...
    try:
        ...
        # telegram.ext without it must not fail the whole ensure.
        try:
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _register_handlers(self, app) -> None:
        ...
        # Stale-update guard, group -1 so it runs before every handler above:
        # connect() preserves Telegram's server-side queue across restarts
        # (drop_pending_updates=False), and this guard bounds the replay
        # window by dropping messages older than the cutoff. Fresh messages
        # and non-message updates (callbacks, edits) pass through.
        # TypeHandler is None only under stub telegram.ext modules that lack
        # it — skip the guard there.

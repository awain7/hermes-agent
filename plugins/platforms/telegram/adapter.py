...
try:
    ...
    # Imported separately (fork): minimal test stubs that register a bare ModuleType as telegram.ext
    # lack ApplicationHandlerStop; degrade to "no stale-update guard" instead of losing telegram.
    try:
...
def check_telegram_requirements() -> bool:
    ...
    try:
        ...
        ApplicationHandlerStop = getattr(_ext, "ApplicationHandlerStop", Exception)  # optional, see module top
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    # Album items are downloaded BEFORE they join the media-group buffer, so this debounce must cover
    # the download time of the NEXT item, not just Telegram's update fan-out latency (fork). At 0.8s a
    # two-file album of full-size image documents routinely split (second download ~1.5s behind the
    # first), so the first image started an agent run alone and the second landed on a busy session
    # and was never analyzed. 3s absorbs realistic download times; the timer restarts on each
    # arriving item, and single (non-album) media never enters this path.
    ...
    # instead of processing it (fork). connect() always preserves the server-side getUpdates queue
    # (drop_pending_updates=False even on a cold boot), so a message sent during a routine restart
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._pending_message_max_age_s: float = self._env_float_clamped(
            "HERMES_TELEGRAM_PENDING_MESSAGE_MAX_AGE", self._PENDING_MESSAGE_MAX_AGE_S, min_value=0.0)
        ...
        # monotonic timestamps of recent heartbeat resurrections, for the rate limit in _on_heartbeat_task_done
    ...
    async def _start_polling_resilient(self, *, drop_pending_updates: bool, error_callback, require_progress: bool = False) -> bool:
        ...
        try:
            ...
            if self._looks_like_polling_conflict(err):
                # Bootstrap conflicts hit before PTB invokes our polling error_callback, so its
                # _disarm_ptb_retry_loop() path never runs. Disarm here as well (fork); otherwise PTB's
                # own network_retry_loop keeps polling while our recovery task does
                # stop -> sleep -> start_polling, producing a self-inflicted 409 loop in multiplexer mode.
                self._disarm_ptb_retry_loop()
                logger.warning(
                    "[%s] Telegram polling bootstrap conflict; gateway stays alive while conflict retry runs: %s",
                    self._diagnostic_label(), _redact_telegram_error_text(err))
    ...
    def _diagnostic_label(self) -> str:
        """Log-safe adapter identity for multi-profile Telegram debugging (fork)."""
    ...
    # The heartbeat loop is the watchdog for the polling connection, but a watchdog nobody watches is
    # a single point of failure: if the task dies from an unexpected exception (or exits via a
    # defensive return while the adapter is still live), every wedge thereafter goes undetected and
    # the bot silently stops receiving messages until a manual restart. Observed 2026-08-24 ~17:00:
    # 75 minutes of total log silence on a wedged profile, zero heartbeat warnings. These helpers
    # (fork) make the heartbeat self-resurrecting, rate-limited so a hard-crashing loop cannot spin.
    ...
    def _on_heartbeat_task_done(self, task: "asyncio.Task") -> None:
        ...
        Deliberate exits stay final: cancellation (disconnect/reconnect paths cancel the task before
        replacing or dropping it) and exits while the adapter has a fatal error or is no longer
        connected. Anything else (an unexpected exception, or a defensive ``return`` inside the loop
        while polling is still supposed to be running) gets logged loudly and the loop is re-armed,
        rate-limited to ``_HEARTBEAT_RESTART_LIMIT`` restarts per ``_HEARTBEAT_RESTART_WINDOW_S``.
        """
        if task.cancelled():
            return
        ...
        while True:
            try:
                ...
                if self.has_fatal_error:
                    logger.info(
                        "[%s] Polling heartbeat exiting: adapter has a fatal error (reconnect queue owns recovery now)",
                        self.name)
                    return
                ...
                if not callable(getattr(bot, "get_me", None)):
                    # Log it (fork): if this fires in production the polling watchdog is about to go dark,
                    # and that must never happen silently again (the resurrection callback brings the
                    # loop back if the adapter still believes it is connected).
                    logger.warning(
                        "[%s] Polling heartbeat exiting: bot object has no callable get_me (app torn down?)",
                        self.name)
                    return
        ...
        if self._polling_conflict_count <= MAX_CONFLICT_RETRIES:
            ...
            try:
                ...
                if self._teardown_started:
                    return
                logger.warning(
                    "[%s] Telegram polling retry %d/%d failed: %s. Scheduling next attempt.", self._diagnostic_label(),
    ...
    def _register_handlers(self, app) -> None:
        ...
        # Stale-update guard (fork), group -1 so it runs before every handler above: connect() preserves
        # Telegram's server-side queue across restarts (drop_pending_updates=False), and this guard bounds
        # the replay window by dropping messages older than the cutoff. Fresh messages and non-message
        # updates (callbacks, edits) pass through. Skipped under stub telegram.ext modules.
        if TypeHandler is not None and TypeHandler is not Any:
            app.add_handler(TypeHandler(Update, self._drop_stale_pending_update), group=-1)
    ...
    async def _start_webhook_mode(self, webhook_url: str, *, is_reconnect: bool) -> None:
        ...
        await self._app.updater.start_webhook(
            ...
            drop_pending_updates=False,  # push-based ⇒ practically a no-op; mirrors polling's always-preserve (fork)
       )
    ...
    async def _start_polling_mode(self, *, is_reconnect: bool) -> None:
        ...
        polling_started = await self._start_polling_resilient(
            # Always preserve the server-side Bot API queue, cold boot included (fork). A cold boot after the
            # nightly auto-update, a crash, or a reboot is exactly when users have messages queued from the
            # downtime window, and the old drop-on-first-boot behavior silently discarded them. The
            # stale-update guard (_drop_stale_pending_update, group -1) bounds the replay after a *long*
            # outage by dropping messages older than the age cutoff, which is what #46621's blanket drop
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        ``is_reconnect`` only affects bookkeeping/logging (fork): both paths preserve the server-side Bot API
        queue (drop_pending_updates=False), because a cold boot after the nightly auto-update, a crash, or a
        reboot is exactly when users have messages queued; the stale-update guard
        (_drop_stale_pending_update) bounds the replay via HERMES_TELEGRAM_PENDING_MESSAGE_MAX_AGE
        (default 600s). Webhook env: TELEGRAM_WEBHOOK_URL,
        ...
        try:
            ...
            if not self._webhook_mode:
                ...
                self._arm_polling_heartbeat()  # self-resurrecting (fork)
    ...
    async def _drop_stale_pending_update(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """group=-1 guard (fork): drop queued inbound messages older than the cutoff.

        Runs before every group-0 handler. connect() preserves the server-side getUpdates queue across
        restarts, so this is the only thing standing between a long outage and a blind replay of
        hours-old commands. Scope is deliberately narrow: callback queries pass through untouched
        (pressing a button on an old message is a legitimate, *current* action); edited messages are
        aged by ``edit_date`` (the edit is the fresh intent); anything without a usable timestamp
        passes through, because dropping real traffic is the worse error.
        """
        ...
        logger.info(
            ...
            self.name, getattr(getattr(message, "chat", None), "id", "?"), age, max_age)

...
_SPECS = [
    ...
    _cmd("block", [
        _TASK_ID,
        # Named flag (fork): a bare positional reason can't coexist with --kind/--ids in argparse
        # without ordering ambiguity ("unrecognized arguments"); same convention as 'unblock'.
        _reason("Reason (also appended as a comment). Quote multi-word reasons."),
        _bulk_ids("block"),
    ...
    _cmd("schedule", [
        _TASK_ID,
        _reason("Reason/timing note (also appended as a comment). Quote multi-word reasons."),
        _bulk_ids("schedule"),

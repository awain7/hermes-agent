...
def _active_profile_name() -> str:
    """Multiplex profile this CLI invocation runs as, or "" for no scoping (fork).

    Derived from HERMES_HOME (``hermes --profile <name>`` points it at the profile's home).
    ``"custom"`` (an unrecognised HERMES_HOME) and any lookup failure fall back to "", which keeps
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    # Profile view (fork): drops other multiplex profiles' tagged entries and collapses the per-profile
    # copies of one shared DM into a single line. Optional so a stand-in channel_directory module
    # without the helper (tests) keeps the raw listing.
    try:

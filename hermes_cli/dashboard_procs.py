...
def _iter_process_table() -> list[tuple[int, str]]:
    ...
    if sys.platform == "win32":
        ...
        if result is None or result.returncode != 0 or not (result.stdout or ""):
            # wmic was removed from Windows 11 (and late Windows 10 builds): the spawn fails (result None)
            # or prints nothing, and this scan used to return [] right here, so `hermes dashboard --stop`
            # / `--status` and the stale-dashboard reaper in `hermes update` silently found nothing on
            # such hosts. Fall back to PowerShell Get-CimInstance (fork), emitting the same LIST-style
            # lines so the parser below does not branch. Mirrors hermes_cli.gateway._scan_gateway_pids.

...
def resolve_send_target(
    ...
    try:
        ...
        # Multiplex: scope name resolution to the active session's profile so
        # a secondary profile's send resolves against its own directory
        # entries, not the default profile's (fork profile-scope series).
        resolved = resolve_channel_name(
            platform_name, target_ref, _current_session_profile()
        )
    except Exception:
        resolved = None

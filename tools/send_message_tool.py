...
from tools.send_message_targets import (
    _HOME_CHANNEL_ENV_OVERRIDES, _SLACK_USER_ID_RE, _current_session_profile, resolve_send_target)
...
def _handle_list():
    try:
        ...
        # Scoped to the active multiplex profile (fork); "" (single-profile) keeps every entry.
        return json.dumps({"targets": format_directory_for_display(profile=_current_session_profile())})

...
def _current_session_profile() -> str:
    """Return the multiplex profile this tool call is running as, or "" (fork).

    Reads ``HERMES_SESSION_PROFILE`` (set on the live gateway's contextvar for the duration of a chat
    turn, see ``gateway/run.py::_set_session_env``) falling back to the ``HERMES_PROFILE`` env var
    (set by the kanban dispatcher when it spawns a worker subprocess). Empty string for
    non-multiplex gateways / standalone CLI, which callers treat as "no filtering" to keep
    pre-multiplex behavior unchanged.
    """
    try:
...
def resolve_send_target(
    ...
    try:
        ...
        # Multiplex (fork): scope name resolution to the active session's profile so a secondary
        # profile's send resolves against its own directory entries, not the default profile's.

...
def _housekeeping_channel_directory(adapters, loop, profile_adapters=None) -> None:
    ...
    if loop is not None:
        ...
        fut = safe_schedule_threadsafe(
            build_channel_directory(adapters, profile_adapters), loop, logger=logger,
...
def _start_gateway_housekeeping(
    ...
    chores += [
        (5, "Channel directory refresh",
         lambda: adapters and _housekeeping_channel_directory(adapters, loop, profile_adapters)),
...
def _start_gateway_start_cron_and_housekeeping(runner):
    ...
    housekeeping_thread = threading.Thread(
        ...
        kwargs={"adapters": runner.adapters, "loop": asyncio.get_running_loop(),
                ...
                # Multiplex (fork): the periodic channel-directory refresh must include every
                # profile's adapters, or secondary profiles' targets vanish five minutes after startup.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Fork: under multiplexing the channel directory is ONE shared file in the launch (default)
    # home; secondary profiles' entries are tagged with a "profile" key by
    # build_channel_directory(adapters, profile_adapters) and filtered on read. Upstream resolves
    # DIRECTORY_PATH lazily from the per-turn HERMES_HOME, which would make a secondary profile's
    # turn read its own (never written) home and lose every send_message name resolution. Pin the
    # explicit overrides once at launch (they default to None) so all profiles read the directory
    # the housekeeping thread writes.
    if getattr(runner.config, "multiplex_profiles", False):
        try:
            from gateway import channel_directory as _channel_directory

            _launch_home = get_hermes_home()
            if _channel_directory.DIRECTORY_PATH is None:
                _channel_directory.DIRECTORY_PATH = _launch_home / "channel_directory.json"
            if _channel_directory.CHANNEL_ALIASES_PATH is None:
                _channel_directory.CHANNEL_ALIASES_PATH = _launch_home / "channel_aliases.json"
        except Exception:
            logger.debug("could not pin the shared multiplex channel directory", exc_info=True)

...
class TurnRunner:
    ...
    def _build_fresh_agent(self, turn_route, platform_key, combined_ephemeral, max_iterations,
        ...
        return ctx.AIAgent(
            ...
            # Multiplex gateway profile this turn runs under (fork): stamped into the system prompt so
            # the stored-prompt reuse check can reject a prompt built under a different profile.
            profile=getattr(src, "profile", None) or None,
    ...
    def run_sync(self):
        ...
        # Profile-scoped, not the init-time snapshot (fork): a secondary multiplex profile's turn
        # must read its own provider_routing.
        pr = runner._resolve_provider_routing()

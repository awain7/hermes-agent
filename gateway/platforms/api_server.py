...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _create_agent(
        ...
        # Multiplex profile for this request (fork). Passed EXPLICITLY because _create_agent runs on a
        # run_in_executor thread, where the _api_request_profile ContextVar set by the /p/<profile>/
        # middleware is not visible. Falls back to the ContextVar for event-loop-thread callers.
        ...
        agent_kwargs = {
            ...
            "platform": "api_server",
            # Multiplex profile selected by the /p/<profile>/ URL prefix (fork). This path passes a
            # persisted session_id AND stored conversation_history, so the stored-prompt restore branch
            # and _stored_prompt_matches_runtime() really execute here; every profile shares one
            # state.db, so an unstamped reader could reuse a prompt built under another profile.

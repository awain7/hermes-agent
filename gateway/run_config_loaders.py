...
class GatewayConfigLoadersMixin:
    ...
    def _resolve_provider_routing(self) -> dict:
        """Provider routing for the ACTIVE profile scope (fork profile-scope series / upstream #57492).

        ``self._provider_routing`` is resolved once in ``__init__`` -- before any per-turn profile
        scope exists -- so it can only ever reflect the default profile's config. During a secondary
        multiplex profile's turn (``_profile_runtime_scope`` active) read and cache that profile's own
        ``provider_routing`` instead; caching per profile home mirrors the init-time snapshot.
        """
        from gateway.run import _gateway_config_home, _hermes_home
    ...
    def _refresh_fallback_model(self) -> list | None:
        ...
        """
        # Profile-scoped (fork): read the ACTIVE profile's config so a secondary multiplex profile's
        # turn refreshes its own chain. The last-known-good cache is kept per profile home -- a single
        # shared slot would leak one profile's chain into another's turn on a transient read failure.
        # ``self._fallback_model`` stays the default profile's slot (tests + init snapshot use it), so
        # single-profile behavior and the #60955 semantics are unchanged.
        from gateway.run import _gateway_config_home, _hermes_home
        home = _gateway_config_home()
